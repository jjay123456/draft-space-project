      ...
      <section className="py-20 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <motion.div
            ...
            transition={{ duration: 0.6 }}
            viewport={{ once: true }}
            className="bg-card/30 backdrop-blur-sm rounded-2xl p-8 border border-primary/10 mb-12"
          >
            <h3 className="text-xl font-semibold text-center mb-6 text-foreground">Progress Over 12 Lessons</h3>
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={outcomesProgressData}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--primary) / 0.1)" />
                <XAxis dataKey="lesson" tick={{ fill: 'hsl(var(--foreground))' }} label={{ value: 'Lesson Number', position: 'insideBottom', offset: -5, fill: 'hsl(var(--muted-foreground))' }} />
                <YAxis tick={{ fill: 'hsl(var(--muted-foreground))' }} label={{ value: 'Progress (%)', angle: -90, position: 'insideLeft', fill: 'hsl(var(--muted-foreground))' }} />
                <Tooltip 
                  contentStyle={{ 
                    backgroundColor: 'hsl(var(--card))',
                    border: '1px solid hsl(var(--primary) / 0.2)',
                ...
                />
                <Legend />
                <Line type="monotone" dataKey="listening" stroke="hsl(var(--primary))" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} name="Listening" />
                <Line type="monotone" dataKey="pronunciation" stroke="hsl(var(--secondary))" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} name="Pronunciation" />
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: Brain, title: "Develop English Thinking Habits", desc: "Learn to think directly in English instead of translating, helping you understand and respond faster in real conversations." },
              { icon: Mic, title: "Enhance Auditory and Speaking Abilities", desc: "With clear, high-quality sound and structured practice, improve listening, pronunciation, and speaking confidence step by step." },
              { icon: Zap, title: "Build Confidence in English Communication", desc: "Become more comfortable using English in everyday situations. Speak naturally, listen better, and enjoy conversations." }
            ].map((outcome, index) => (
              <motion.div
                key={outcome.title}
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                <Card className="h-full border-primary/10 bg-gradient-to-br from-card/80 to-card/40 backdrop-blur-sm hover:shadow-xl transition-all duration-300">
                  <CardContent className="p-6 text-center">
